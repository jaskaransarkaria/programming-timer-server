...
// TODO: the mocked session contains real timestamp
...
func TestHandlePauseSession(t *testing.T) {
	// take an existing session
	_, _, sessionIndex, mockConnInitUser := setup()
	// add another user (so we can verify that the function is switching driver correctly
	sessionID := fmt.Sprintf("mocked-id-%d", sessionIndex)
	...
	testSession, _ := JoinExistingSession(sessionToJoin, newUser)
	mockPauseRequest := PauseRequest{
		SessionID: testSession.SessionID,
		PauseTime: testSession.EndTime - (testSession.Duration / 2),
	}

	mockPauseResponse := PauseSessionResponse{
		PauseTime: mockPauseRequest.PauseTime,
	}
	// mock broadcast to all sessionUsers
	mockConnInitUser.On("WriteJSON", mockPauseResponse).Return(nil)
	mockConnJoiningUser.On("WriteJSON", mockPauseResponse).Return(nil)

	HandlePauseSession(mockPauseRequest)
}

func TestHandleUnpauseSession(t *testing.T) {
	// take an existing session
	_, _, sessionIndex, mockConnInitUser := setup()
	// add another user (so we can verify that the function is switching driver correctly
	sessionID := fmt.Sprintf("mocked-id-%d", sessionIndex)
	...
	mockUnpauseRequest := UnpauseRequest{
		SessionID:   sessionID,
		UnpauseTime: testSession.EndTime - (testSession.Duration / 4),
	}

	mockUnpauseResponse := UnpauseSessionResponse{
		UnpauseTime: testSession.EndTime - (testSession.Duration / 4),
	}

	mockConnInitUser.On("WriteJSON", mockUnpauseResponse).Return(nil)
	mockConnJoiningUser.On("WriteJSON", mockUnpauseResponse).Return(nil)
	HandleUnpauseSession(mockUnpauseRequest)
